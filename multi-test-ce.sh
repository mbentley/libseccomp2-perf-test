...
echo -e "\nAverage stats from ${NUM_LOOPS} loops of ${NUM_EXECS} execs on docker-ce=$(dpkg -l docker-ce | grep ^ii | awk '{print $3}') for libseccomp2=$(dpkg -l libseccomp2 | grep ^ii | awk '{print $3}') with DISABLE_SECCOMP=${DISABLE_SECCOMP}:"
...
# check to sce if we need to write headers to the csv output
if [ ! -f "test_results.csv" ]
then
  echo "docker-ce version,libseccomp2 version,num_loops,num_execs,disable_seccomp,min,max,avg" > test_results.csv
fi

# write output to a log file
echo "docker-ce=$(dpkg -l docker-ce | grep ^ii | awk '{print $3}'),libseccomp2=$(dpkg -l libseccomp2 | grep ^ii | awk '{print $3}'),${NUM_LOOPS},${NUM_EXECS},${DISABLE_SECCOMP},$(awk '{sum+=$2}END{printf "%0.2f\n",sum/NR}' stats_min.txt),$(awk '{sum+=$2}END{printf "%0.2f\n",sum/NR}' stats_max.txt),$(awk '{sum+=$2}END{printf "%0.2f\n",sum/NR}' stats_avg.txt)" >> test_results.csv
